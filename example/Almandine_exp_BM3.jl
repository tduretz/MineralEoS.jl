...
let
    ...
    params = assign_EoS_parameters(:Almandine; sc=scales)
    ...
    function Visualisation()
        ...
        ax = Axis(fig[1,1], xlabel=L"$T$ (K)", ylabel=L"$P$ (GPa)", title=L"$$Diamond - BM3 Einstein model ")
        hm = heatmap!(ax, T * scales.T, P * scales.σ./1e9, ρ_BM3' * ρc)
        Colorbar(fig[1,2], hm, label = L"$ρ$ (kg/m³)" )

        ax = Axis(fig[2,1], xlabel=L"$T$ (K)", ylabel=L"$P$ (GPa)", title=L"$$Diamond - exponential model")
